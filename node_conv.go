...
func (c *nodeConv) decls(begin, end token.Pos, from []ast.Decl) []Declaration {
	if len(from) == 0 {
		return nil
	}
	...
	for i, f := range from {
		...
		to[i] = c.nodePos(declBegin, declEnd, f).(Declaration)
	}
	return to
}
...
func (c *nodeConv) markup(begin, end token.Pos, n ast.Node) Context {
	...
	for ; i < l; i++ { // TODO: but need to tell if first in list! -- for len(groups) > 0 { group := groups[0]; if group ... then break else groups = groups[1:]
		...
		for j, comment = range group.List {
			...
			before = append(before, c.node(comment).(Gap))
		}
	}
	if comment != nil {
	...
	if len(groups) > 0 {
		...
		for i, l = 0, len(groups); i < l; i++ {
			...
			for j, comment = range group.List {
				...
				after = append(after, c.node(comment).(Gap), &Line{})
			}
		}
	}
...
func (c *nodeConv) nodePos(begin, end token.Pos, n ast.Node) (to Syntax) {
	switch n := n.(type) {
	// case *ast.AssignStmt:
	// 	return &Assign{
	// 		// Left: c.exprs(n.Lhs),
	...
	case *ast.File:
		...
		// imports
		// decls
		c.comments = ast.NewCommentMap(c.fileSet, n, n.Comments)
		c.file = c.fileSet.File(n.Pos())
		file := &File{}
		file.Context = c.markup(c.file.Pos(0), c.file.Pos(c.file.Size()), n)
		var p token.Pos
		if len(n.Decls) == 0 {
			p = c.nodeEnd(n.Name)
		} else {
		...
		file.Package = c.nodePos(add(n.Package, lenPackage), p, n.Name).(*Name)
		if len(n.Decls) > 0 {
			file.Decls = c.decls(p, c.nodeEnd(n.Decls[len(n.Decls)-1]), n.Decls)
		}
		to = file
		// case *ast.Package:
...
func (c *nodeConv) stmts(begin, end token.Pos, from []ast.Stmt) []Statement {
	...
	for i, f := range from {
		...
		to[i] = c.nodePos(stmtBegin, stmtEnd, f).(Statement)
	}
	return to
}
